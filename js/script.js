...
.then(resp => {
        ...
        preQuestions = resp;
        document.getElementById("questionsLength").innerHTML=resp.length;
        setQuestion(0);
        clearAnswered();
    });
...
let answered = [preQuestions.length];
...
function setQuestion(index) {
 ...
 next.addEventListener('click', function () {
    if(index >= preQuestions.length-1){
        ...
        average.innerHTML = parseInt(localStorage.getItem("sum"))/parseInt(localStorage.getItem("count"));
    }else{
        index++;
        setQuestion(index);
...
function doAction(event) {
    ...
    if(answered[index].answered===false){
        ...
        for(let i=0; i<answers.length; i++){
            if (preQuestions[index].answers[i] === event.target.innerHTML){
                answers[i].style.backgroundColor = 'red';
            }
            if (preQuestions[index].answers[i] === preQuestions[index].correct_answer){
...
function colorAnswers(){
    if(answered[index].answered===true){
        for(let i=0; i<answers.length; i++){
            if(preQuestions[index].answers[i]===answered[index].usersAnswer){
                answers[i].style.backgroundColor = 'red';
            }
        }
        for(let i=0; i<answers.length; i++){
            if(preQuestions[index].answers[i]===answered[index].correctAnswer){
                answers[i].style.backgroundColor = 'green';
            }
        }
        for(let i=0; i<answers.length; i++){
            if(preQuestions[index].answers[i]!=answered[index].correctAnswer && preQuestions[index].answers[i]!=answered[index].usersAnswer){
...
function markCorrect(ev){
    answered[index].usersAnswer = ev.innerHTML;
    answered[index].correctAnswer = ev.innerHTML;
}

function markInCorrect(ev){
    answered[index].usersAnswer = ev.innerHTML;
    answered[index].correctAnswer = preQuestions[index].correct_answer;
}

function disableAnswers(){answered[index].answered=true;}

function clearClass(){
    if(answered[index+1].answered===false){
...
function saveInMemory(){
    if(localStorage.getItem("count")==null){
        ...
        let gamesCount = parseInt(localStorage.getItem("count"),10)+1;
        localStorage.setItem("count", gamesCount);
    }
    
    if(localStorage.getItem("sum")==null){
        localStorage.setItem("sum", points);
    }else{
        let pointsSum = parseInt(localStorage.getItem("sum"),10)+points;
        localStorage.setItem("sum", pointsSum);
    }
}
